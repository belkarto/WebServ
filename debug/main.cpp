...
int main()
{
    ...
    while (1)
    {
        ...
        if ((serversPool.ready_size = select(serversPool.max_socket, &serversPool.workinSet, NULL, NULL, NULL)) < 0)
            errAnExit("select()");
        for (int i = 0; i < serversPool.max_socket && serversPool.ready_size > 0; i++)
        {
            ...
            if (FD_ISSET(i, &serversPool.workinSet))
            {
                ...
                if ((connFd = accept(i, NULL, NULL)) < 0)
                    errAnExit("accept()");
                std::cout << serversPool.max_socket << "---------------------------------------------" << std::endl;
